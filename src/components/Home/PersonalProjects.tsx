...
export default function PersonalProjects({ language }: PersonalProjectsProps) {
  ...
  return (
    <section id="projects" className="mb-20 scroll-mt-24">
      <Title>{t.home.personalProjects.title}</Title>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {t.home.personalProjects.items.map((project, idx) => (
          <div
            key={idx}
            className="card rounded-lg p-6 group cursor-pointer animate-on-scroll hover:scale-[1.02] transition-all"
            style={{ animationDelay: `${idx * 0.15}s` }}
          >
            <h3 className="h3-projects text-2xl font-bold">
              {project.title}
            </h3>
