    ...
    <section id="projects" className="mb-20 scroll-mt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {t.projects.items.map((project, idx) => (
          <div 
            key={idx} 
            className="cyber-card rounded-lg p-6 group cursor-pointer animate-on-scroll"
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {project.tags.map((tag, i) => (
                <span 
                  key={i} 
                  className="px-3 py-1 text-sm font-mono bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 rounded"
                >
                  {tag}
                </span>
              ))}
